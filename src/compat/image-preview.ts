// Adapted from GPT Web Codex v2.2.11 (MIT). The app-only restore tool remains private.
export const IMAGE_PREVIEW_RESOURCE_URI = "ui://kai-work-host/image-preview-v1.html";
...
<body>
  <main id="card" class="card" hidden>
    <div class="stage"><img id="preview" alt="本機圖片預覽"></div>
    <div class="meta"><span id="name" class="name"></span><span id="detail" class="detail"></span></div>
  </main>
  <div id="status" class="status">
    <div id="statusText">正在準備圖片預覽…</div>
    <button id="retry" class="retry" type="button" hidden>重新載入圖片</button>
  </div>
  <script>
    (() => {
      ...
      const renderImage = (image) => {
        ...
        preview.alt = "本機圖片預覽：" + (image.name || "image");
      ...
      const showRestoring = () => {
        if (rendered) return;
        status.hidden = false;
        statusText.textContent = "正在還原圖片預覽…";
        retry.hidden = true;
      };
      const showRestoreError = (error) => {
        if (rendered) return;
        status.hidden = false;
        statusText.textContent = "圖片預覽還原失敗：" + (error?.message || String(error));
      ...
      const restorePreview = async (previewId) => {
        ...
        try {
          ...
          if (!renderFrom(result)) throw new Error("本機快取未傳回可顯示的圖片");
        } catch (error) {
      ...
      const renderFrom = (globals) => {
        ...
        if (job?.status === "completed") statusText.textContent = "任務已完成，但未傳回可顯示的圖片。";
        else if (job?.status) statusText.textContent = "Luna 任務狀態：" + job.status + "，正在等待圖片產物…";
        return false;
      };
      ...
      preview.addEventListener("error", () => {
        rendered = false;
        card.hidden = true;
        showRestoreError(new Error("圖片資料無法解碼"));
      });
      ...
      window.addEventListener("message", (event) => {
        ...
        if (message.id !== undefined && pendingRequests.has(message.id)) {
          ...
          if (message.error) pending.reject(new Error(message.error.message || "Host 要求失敗"));
      ...
      const finishStartup = () => {
        ...
        setTimeout(() => {
          if (!rendered && !restoringPreviewId && !lastPreviewId) {
            ...
            statusText.textContent = "圖片預覽資料無法使用，請重新呼叫圖片預覽工具。";
          }
        }, 2000);
      };
